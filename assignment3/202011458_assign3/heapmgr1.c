...
merge_chunk(Chunk_T c1, Chunk_T c2)
{
   ...
   chunk_set_next_free_chunk(chunk_get_footer_from_header(c2), chunk_get_next_free_chunk(chunk_get_footer_from_header(c1)));
   if(chunk_get_next_free_chunk(c2) != NULL){
      chunk_set_next_free_chunk(chunk_get_footer_from_header(chunk_get_next_free_chunk(c2)), c1);
   }
   return c1;
}
/*--------------------------------------------------------------------*/
...
split_chunk(Chunk_T c, size_t units)
{
   ...
   /* adjust the pointer of chunk */
   chunk_set_next_free_chunk(c2, chunk_get_next_free_chunk(c));
   chunk_set_next_free_chunk(c_footer, chunk_get_next_free_chunk(chunk_get_footer_from_header(c2)));
   chunk_set_next_free_chunk(chunk_get_footer_from_header(c2), c);
...
insert_chunk(Chunk_T c)
{
   ...
   if (g_free_head == NULL) {
      ...
      chunk_set_next_free_chunk(chunk_get_footer_from_header(c), NULL);
      chunk_set_next_free_chunk(chunk_get_footer_from_header(g_free_head), c);
